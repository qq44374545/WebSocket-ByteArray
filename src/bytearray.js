...
/**
 * @param bytes 类型为ArrayBuffer类型的字节数组
 */
var ByteArray = function (bytes) {
    ...
    var _position = 0;
    var _dv = new DataView(bytes);
    var _byteLength = bytes.byteLength;

    /**
     *  @param length 扩充的容器长度,单位 byte
     *  扩充容器
     */
    this.plusCapacity = function (length) {

        var plusLength = _byteLength;
        ...
        //扩充容器
        var bytesNew = new ArrayBuffer(bytes.byteLength + length);
        var dvNew = new Uint8Array(bytesNew);

        //创建容器副本
        var dvCopy = new Uint8Array(bytes);
        dvNew.set(dvCopy, 0);
        bytes = dvNew.buffer;
    ...
    this.writeBytes = function (data) {
        ...
        if (length > 0) {
            ...
            var bytesDV = new Uint8Array(bytes);
            bytesDV.set(dataDV, _position);
            _position += dataDV.byteLength;
            _byteLength += dataDV.byteLength;
        }
    }.bind(this)
...
//----------------------------------
    ...
    this.readByte = function () {
        _position++;
        return _dv.getInt8(_position - 1, false);
    }.bind(this)
    ...
    this.readBytes = function (length) {
        ...
        if ((_position + length)  > _byteLength) {
            throw Error(' Error : Unable to read the data of length . [readBytes byteLength=' + _byteLength + "]");
        }
    ...
    this.buffer = function () {
        return this.slice(0, _byteLength);
    }.bind(this)
    ...
    this.slice = function (begin, end) {
        if (begin === undefined) {
            throw Error("Invalid args\nUsage: slice(begin,end).\n");
        }
        end = end === undefined ? _byteLength : end;
